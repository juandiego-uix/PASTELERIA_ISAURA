...
select("#save-backup").addEventListener("click", async () => { try { const response = await api("/api/admin/backup/save", { method: "POST" }); showToast(response.message || "Respaldo guardado"); } catch (error) { showToast(`No se pudo guardar el respaldo: ${error.message}`); } });
